...
const _html2canvas = require("html2canvas-pro");
...
const printers = async (id = null) => {
  if (id != null) {
    ...
    const result = await (0, tauri_1.invoke)(
...
const print = async (data, options) => {
  ...
  for (const item of data) {
    if (item.type == "image") {
      ...
      image.src = `data:${mime_1.default.getType(
        item.url
      )};base64,${buffer_1.Buffer.from(response.data).toString("base64")}`;
      if (item.width) {
  ...
  const tempPath = await (0, tauri_1.invoke)(
    "plugin:printer|create_temp_file",
    {
  ...
  if (printerid != null) {
    ...
    for (const job of listRawJobs) {
      ...
      allJobs.push({
        ...
        job_id: job.Id,
        job_status:
          constants_1.jobStatus[job.JobStatus] != undefined
            ? {
                code: job.JobStatus,
